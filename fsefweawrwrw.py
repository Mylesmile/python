...
class Gallery :
    ...
    def img_insert(self, text_box) :
        ...
        f.write("%s, %s/n" % (photo_info[:-1], self.date))
    ...
    def img_show(self, event) :
        ...
        for line in data :
            ...
            if line[0] == photoname :
                ...
                (line[0], memo, line[2])
            self.lbl_info['text'] = se;f.string
            f.close()
...
text_box = Text( win,  text = 10, width = 40 )

save_btn = Button( win, text = "save", font = ("consolas", 15), bg = "yellow", fg = "red", command = lambda : setImg.img_insert(text_box) )


gallery = Gallery(img_list, img_lbl, info_lbl, file_name)


img_list.bind("<Double-Button-1>", galley.img_show)


img_lbl.girid( row = 0, column = 0 )
img_list.grid(row = 0, column = 1, columnspan = 2)
info_lbl.grid(row = 1, colum = 0, rowspan = 3)
